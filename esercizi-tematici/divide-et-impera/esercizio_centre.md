...
Con il Master Theorem abbiamo $a=1, b=2$ e $f(n) = \Theta(1)$ 
Dato che $n^{\log_b a} = n^{\log_2 1} = n^0 = 1 = \Theta(1)$, il lavoro fatto per combinare le chiamate ricorsive e il termine 
$n^{\log_b a}$ crescono in modo asintoticamente equivalente. Si conclude quindi, tramite il Caso 2 del Master Theorem, che la complessità è:
